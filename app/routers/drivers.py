...
def get_drivers(db: Session = Depends(get_read_db), current_user = Depends(get_current_user)):
    return db.query(Driver).all()

@router.get("/{abbreviation}", response_model=DriverSchema)
def get_driver(abbreviation: str, db: Session = Depends(get_read_db), current_user = Depends(get_current_user)):
